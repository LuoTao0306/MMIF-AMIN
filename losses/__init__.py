...
class EntropyLoss(nn.Module):
    ...
    def forward(self, img):
        # 对输入图像进行四舍五入并转换为 uint8 类型
        img = torch.round(img).to(torch.uint8)
        # 展平图像
        flat_img = img.flatten()
        # 计算每个灰度级别的像素数量
        counts = torch.bincount(flat_img)
        # 计算每个灰度级别的概率
        probabilities = counts / flat_img.numel()
        # 避免对数运算中出现零值
        probabilities = probabilities + (probabilities == 0).float()
        # 计算熵
        entropy = -torch.sum(probabilities * torch.log2(probabilities))
        return entropy
def AG_loss(x):
    AG_loss = AverageGradientLoss().to(device)
    AG = AG_loss(x)
    return AG
def EN_loss(x):
    EN_loss = EntropyLoss().to(device)
    EN = EN_loss(x)
    return EN
def MP_AG_loss(MRI,PET):
    mri = AG_loss(MRI)
    pet = AG_loss(PET)
    add = pet + mri
    return mri/add, pet/add

def MP_EN_loss(MRI,PET):
    mri = EN_loss(MRI)
    pet = EN_loss(PET)
    add = pet + mri
    return mri/add, pet/add
...
def RMI_mri (input_vi,fused_result ):
    RMI_vi=RMI(input_vi,fused_result)

    return RMI_vi
def RMI_spect (input_ir,fused_result ):
    RMI_ir=RMI(input_ir,fused_result)

    return RMI_ir

def ssim_mri (fused_result,input_vi ):
    ssim_vi=ssim(fused_result,input_vi)

    return ssim_vi

def ssim_spect (fused_result,input_ir ):
